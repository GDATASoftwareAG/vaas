...
VAAS_HOST = urlparse(VAAS_URL).hostname
...
class TestVaas:
    ...
    @pytest.mark.parametrize(
        ...
        [(EICAR_SHA256, "Malicious"), (CLEAN_SHA256, "Clean"), (PUP_SHA256, "Pup")],
    ...
    async def test_for_sha256_send_options(
        self, vaas, use_cache, use_hash_lookup, httpx_mock
    ):
    ...
    @pytest.mark.asyncio()
    async def test_for_sha256_bad_request_raise_vaas_client_error(
        self, vaas, httpx_mock
    ):
        request_url = f"{VAAS_URL}/files/{CLEAN_SHA256}/report?useCache={str(True).lower()}&useHashLookup={str(True).lower()}"
        httpx_mock.add_response(
            method="GET",
            url=request_url,
            status_code=400,
            json={"detail": "Mocked client-side error", "type": "VaasClientException"},
        )
        vaas.authenticator.get_token = AsyncMock(return_value="mocked-token")
    ...
    @pytest.mark.asyncio()
    async def test_for_sha256_server_error_raise_vaas_server_error(
        self, vaas, httpx_mock
    ):
        request_url = f"{VAAS_URL}/files/{CLEAN_SHA256}/report?useCache={str(True).lower()}&useHashLookup={str(True).lower()}"
        httpx_mock.add_response(
            ...
            url=request_url,
            status_code=500,
            json={"detail": "Mocked server-side error", "type": "VaasServerException"},
        )
        vaas.authenticator.get_token = AsyncMock(return_value="mocked-token")
    ...
    @pytest.mark.asyncio()
    async def test_for_sha256_authentication_error_raise_vaas_authentication_error(
        self, vaas
    ):
        vaas.authenticator.get_token = AsyncMock(
    ...
    @pytest.mark.asyncio()
    async def test_for_sha256_unauthorized_raise_vaas_authentication_error(
        self, vaas, httpx_mock
    ):
        request_url = f"{VAAS_URL}/files/{CLEAN_SHA256}/report?useCache={str(True).lower()}&useHashLookup={str(True).lower()}"
        httpx_mock.add_response(
    ...
    @pytest.mark.asyncio()
    @pytest.mark.parametrize(
        "url, expected_verdict, is_encrypted",
        [
            (EICAR_URL, "Malicious", False),
            (CLEAN_URL, "Clean", False),
            (PUP_URL, "Pup", False),
            (PASSWORD_ZIP_URL, "Clean", True),
            (WITH_AND_WITHOUT_PASSWORD_ZIP_URL, "Malicious", True),
        ],
        ids=["Malware", "Clean", "Pup", "Encrypted", "EncryptedWithMalwareInside"],
    )
    async def test_for_stream_returns_verdict(
        ...
        async with httpx.AsyncClient(headers=STREAM_TEST_CLIENT_HEADERS) as client:
            ...
            assert verdict.verdict == expected_verdict
            assert verdict.isEncrypted == is_encrypted

    @pytest.mark.asyncio()
    @pytest.mark.httpx_mock(
        should_mock=lambda request: request.url.host == VAAS_HOST
    )
    @pytest.mark.parametrize(
        "use_hash_lookup",
        [False, True],
    ...
    async def test_for_stream_bad_request_raise_vaas_client_error(
        self, vaas, httpx_mock
    ):
        httpx_mock.add_response(
            method="POST",
    ...
    async def test_for_stream_server_error_raise_vaas_server_error(
        self, vaas, httpx_mock
    ):
        httpx_mock.add_response(
            method="POST",
    ...
    async def test_for_stream_authentication_error_raise_vaas_authentication_error(
        self, vaas, httpx_mock
    ):
        vaas.authenticator.get_token = AsyncMock(
            side_effect=VaasAuthenticationError("Mocked auth error")
    ...
    async def test_for_stream_unauthorized_raise_vaas_authentication_error(
        self, vaas, httpx_mock
    ):
        httpx_mock.add_response(
            method="POST",
    ...
    async def test_for_file_returns_verdict(
        self, vaas, url, expected_verdict, is_encrypted
    ):
    ...
    async def test_for_file_send_options(
        self, vaas, use_cache, use_hash_lookup, request_count, httpx_mock
    ):
        url = CLEAN_URL
        filename = os.path.join("/tmp", os.path.basename(url))
    ...
    async def test_for_file_server_error_raise_vaas_server_error(
        self, vaas, httpx_mock
    ):
        url = CLEAN_URL
        filename = os.path.join("/tmp", os.path.basename(url))
    ...
    @pytest.mark.asyncio()
    async def test_for_file_authentication_error_raise_vaas_authentication_error(
        self, vaas
    ):
        url = CLEAN_URL
        filename = os.path.join("/tmp", os.path.basename(url))
    ...
    async def test_for_file_unauthorized_raise_vaas_authentication_error(
        self, vaas, httpx_mock
    ):
        url = CLEAN_URL
        filename = os.path.join("/tmp", os.path.basename(url))
    ...
    async def test_for_url_returns_verdict(
        self, vaas, url, expected_verdict, is_encrypted
    ):
    ...
    async def test_for_url_bad_request_raise_vaas_client_error(
        self, vaas, httpx_mock, post_fails
    ):
        url = CLEAN_URL
    ...
    async def test_for_url_server_error_raise_vaas_server_error(
        self, vaas, httpx_mock, post_fails
    ):
        url = CLEAN_URL
    ...
    async def test_for_url_authentication_error_raise_vaas_authentication_error(
        self, vaas
    ):
        url = CLEAN_URL
        vaas.authenticator.get_token = AsyncMock(
    ...
    async def test_for_url_unauthorized_raise_vaas_authentication_error(
        self, vaas, post_fails, httpx_mock
    ):
        url = CLEAN_URL
    ...
    @pytest.mark.parametrize(
        "post_fails", [True, False], ids=["post_cancel", "get_cancel"]
    )
    async def test_for_url_cancel_request_raise_cancel_error(
        self, vaas, post_fails, httpx_mock
    ):
        url = CLEAN_URL
