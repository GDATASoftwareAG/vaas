...
public class IntegrationTests
{
    ...
    private static async Task<Vaas> AuthenticateWithCredentials(Uri tokenEndpoint = "https://keycloak-vaas.gdatasecurity.de/realms/vaas/protocol/openid-connect/token")
    {
        DotNetEnv.Env.TraversePath().Load();
        var clientId = DotNetEnv.Env.GetString("CLIENT_ID");
        var clientSecret = DotNetEnv.Env.GetString("CLIENT_SECRET");
        var vaas = new Vaas();
        await vaas.ConnectWithCredentials(clientId, clientSecret, tokenEndpoint);
        return vaas;
    }
}